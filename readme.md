...
This tool aims to help you create chapters for podcasts easily and create chapters from existing podcasts that have video versions.

It provides CLI tools for automatic creation of chapters from videos and a web interface to edit and convert chapters between several formats.
...
In your terminal run `npm i podcasthelper -g`. Next, run `podcasthelper --serve` then click the url that's displayed or open http://localhost:8989/ in your browser
...
```shell
podcasthelper generate --input=YOUR_FILE.mp4
```
...
> `%bin <command> <input?> --option-a --option-b=value`
...
```dotenv
# prefix with PCT_, option name uppercase and replace all dashes with underscore
PCT_CROP="616:410:554:46"
PCT_SILENT=true
PCT_DUMP_FFMEG=true
```
...
You need to install `chaptertool` globally to use the commands as described below.
...
chaptertool generate cropme.webm --crop="926:608:831:72"
```
...
chaptertool generate crossfaded.webm --use-crossfade-fix
```
