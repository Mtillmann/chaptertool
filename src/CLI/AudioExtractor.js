...
export class AudioExtractor {
  ...
  extract () {
    ...
    if ('audioCopyStream' in this.options) {
      const ffProbeArgs = [`-i "${this.options.input}"`, '-v 0 -select_streams a -show_entries "stream=codec_name"']
      ...
      const fileExtension = codecToExtensionMap[codec]
      ...
      if (this.options.overwriteMode === '-n') {
        const outputExtension = extname(this.options.audioFilename)
        this.options.audioFilename = addSuffixToPath(this.options.audioFilename.replace(new RegExp(`${outputExtension}$`), ''), outputExtension)
      }

      const ffmpegArgs = [`-i "${this.options.input}"`, this.options.audioOptions, this.options.overwriteMode, `"${this.options.audioFilename}"`]
